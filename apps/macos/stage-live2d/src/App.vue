...
const subtitleText = computed(() => {
  ...
  return state.statusMessage || "请先连接 OpenClaw 网关。";
});
...
const emptyStateCopy = computed(() => {
  switch (state.connectionStatus) {
    ...
    case "connected":
      return "网关已连接，舞台固定绑定到 `main` 会话。";
    case "error":
      return state.statusMessage || "网关返回了错误。";
    default:
      return "连接一个已有的 OpenClaw 网关后即可开始聊天。";
  }
});
    ...
    <template v-if="!isPetMode">
      <section class="scene-column">
        ...
        <header class="scene-meta">
          ...
          <div class="meta-pill muted">{{ connectionLabel }}</div>
        </header>
      ...
      <aside class="chat-column">
        <header class="chat-header">
          <div class="chat-header__copy">
            ...
            <p class="status-copy">{{ emptyStateCopy }}</p>
          </div>
          <div class="header-actions">
            <span class="presence-chip">{{ presenceLabel }}</span>
            <button
              type="button"
              class="icon-button"
        ...
        <section class="chat-stream">
          <div class="stream-label">助手流</div>
          <div class="stream-copy">{{ streamPreview }}</div>
        </section>

        <section class="message-list">
          <article v-if="renderedMessages.length === 0" class="message-card assistant">
        ...
        <form class="composer" @submit.prevent="submitMessage">
          <label class="composer-label" for="composer-textarea">发送到 `main` 会话</label>
          <textarea
            id="composer-textarea"
            v-model="composerText"
            rows="4"
            placeholder="通过现有的 OpenClaw 网关和 AIRI 对话…"
            ...
            <div class="drawer-sections">
              ...
              <section class="drawer-section">
                ...
                <div class="model-grid">
                  ...
                  >
                    <span class="model-card__swatch" :style="{ background: pack.accentHex }"></span>
                ...
                <div class="drawer-actions">
                  ...
                  <button type="button" class="primary-button" @click="saveCharacter">应用构图</button>
                </div>
              </section>
