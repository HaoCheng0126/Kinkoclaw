...
final class PetOverlayController {
    ...
    func show() {
        ...
        if let window {
            if let origin = self.settings.windowOrigin {
                ...
                window.setFrame(PetWindowPlacement.bottomRightFrame(size: self.size, padding: 18), display: false)
            }
    ...
    func presentContextMenu(with event: NSEvent, for view: NSView) {
        ...
        let packsMenu = NSMenu(title: "Character Pack")
        for pack in PetPackRegistry.packs {
        ...
        let packsItem = NSMenuItem(title: "Character Pack", action: nil, keyEquivalent: "")
